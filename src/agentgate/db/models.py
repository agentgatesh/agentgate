...
class ProcessedEvent(Base):
    ...
    __table_args__ = (
        Index("ix_processed_events_source_id", "source", "event_id", unique=True),
    )
...
class Transaction(Base):
    ...
    payer_org_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("organizations.id", ondelete="SET NULL"),
        nullable=True,
    )
    receiver_org_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("organizations.id", ondelete="SET NULL"),
        nullable=True,
    )
